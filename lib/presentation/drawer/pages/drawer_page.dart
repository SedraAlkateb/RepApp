...
class DrawerPage extends StatelessWidget {
        ...
        children: [
          ...
          Column(
            children: [
                ...
                child: Column(
                  children: [
                    ...
                    Padding(
                      ...
                      child: Row(
                        children: [
                          _buildStatCard("المشافي", UserInfo.numOfHospitalVisit.toString(), Colors.red, true),
                          SizedBox(width: 15.w),
                          _buildStatCard("الأطباء", UserInfo.numOfDoctorVisit.toString(), Colors.transparent, false),
                        ],
                      ),
                    ),
                  ],
              ...
              Expanded(
                child: ListView(
                  padding: EdgeInsets.only(top: 10.h), // مسافة بسيطة بعد الهيدر الثابت
                  children: [
  ...
  Widget _buildDrawerItem(BuildContext context, IconData icon, String title, VoidCallback onTap,{Color? color}) {
    return ListTile(
      leading: Icon(icon, color: color?? Color(0xFF546E7A), size: 22.sp), // لون رمادي أغمق للأيقونات
      title: Text(
        title,
        style: TextStyle(
  ...
  Widget _buildStatCard(String title, String count, Color dotColor, bool showDot) {
    return Expanded(
      child: Container(
        ...
        child: Column(
          children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                if (showDot) Container(width: 6, height: 6, decoration: BoxDecoration(color: dotColor, shape: BoxShape.circle)),
                if (showDot) const SizedBox(width: 4),
                Text(title, style: TextStyle(fontSize: 11.sp, color: Colors.grey[600])),
              ],
            ),
            SizedBox(height: 4.h),
            Text(count, style: TextStyle(fontSize: 18.sp, fontWeight: FontWeight.bold, color: ColorManager.secondaryColor1)),
          ],
        ),
      ),
    );
