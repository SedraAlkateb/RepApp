...
class ReportVisitDoctorBloc
  ...
  ReportVisitDoctorBloc(
      this.allVisitDoctorRepSenUsecase,
      this.readVisitUsecase,
      this.allVisitHospitalRepSenUsecase,
      this.allReadSenUsecase,
      this.visitReadStatus)
      : super(ReportVisitDoctorInitial()) {
    on<AllReportVisitDoctorEvent>((event, emit) async {
    ...
    on<WhoAllReadEvent>((event, emit) async {
      emit(AllWhoAllReadLoadingState());
      (await visitReadStatus.execute(
              event.visitId, event.visitType, event.repType))
          .fold((failure) {
    ...
    on<AllReportVisitHospitalEvent>((event, emit) async {
    ...
    on<SenSearchNoteVisitHospitalEvent>((event, emit) async {
      ...
      hospitalNote = repVisitHospital.where((value) {
        ...
        if (value.samples.any(
          (sample) => normalizeText(sample).contains(search),
        )) {
          return true;
        }
    ...
    on<SenSearchNoteVisitDoctorEvent>((event, emit) async {
      List<RepVisitsModel> doctorNote;

      final String search = normalizeText(event.contant);
    ...
    on<DocIsExpandedNoteEvent>((event, emit) async {
      isExpanded = true;
    ...
    on<ChangeReadDocNoteEvent>((event, emit) async {
    ...
    on<ChangeReadHosNoteEvent>((event, emit) async {
      emit(AsReadLoadingState());
      (await readVisitUsecase.execute(AsRead(
              int.parse(event.repVisitsModel.visitId),
              UserInfo.repId,
    ...
    on<ExpandedBorder>((event, emit) async {
      num = event.num;
