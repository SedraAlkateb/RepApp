...
final _log = AppLogger.get('AsyncBloc');
...
class AsyncBloc extends Bloc<AsyncEvent, AsyncState> {
  ...
  AsyncBloc(
      ...
      : super(AsyncInitial()) {
    on<AsyncDataEvent>((event, emit) async {
      var brands = <BrandModel>[];
      var places = <PlaceModel>[];
      var spec = <SpecDModel>[];
      var doctors = <DoctorModel>[];
      var hospitals = <HospitalModel>[];
      var hospitalSps = <HospitalSpModel>[];
      var brandSpModel = <BrandSpModel>[];
      ...
      try {
        ...
        try {
          ...
          if (doctorsFailureOrSuccess is Failure) {
            ...
            return; // 🛑 توقف فوري عند ارجاع Failure
          }
          ...
          return; // 🛑 إيقاف عملية المزامنة تماماً ومنع استكمال باقي Steps
        }
        ...
        emit(getDataSucState(SyncPayload(
          brands: brands,
          places: places,
          spec: spec,
          doctors: doctors,
          hospitals: hospitals,
          hospitalSps: hospitalSps,
          brandSpModel: brandSpModel,
    ...
    on<SetDataSEvent>((event, emit) async {
      final current = state;
      if (current is! getDataSucState) return;
    ...
    on<PlanIsActiveEvent>((event, emit) async {
      FirebaseCrashlytics.instance
    ...
    on<UpdateRepEvent>((event, emit) async {
      await FirebaseCrashlytics.instance
  ...
  Future<void> logSyncStep({
    ...
    try {
      ...
      _log.warning('Logging Error (Crashlytics/Analytics)', e);
    }
  }
}
